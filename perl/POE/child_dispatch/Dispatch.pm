...
sub process_data
{
    ...
    # if a NS command
    if ($data =~ /Hello NS/) {
        $logger->info("dispatching further requests to NS");

        # unset the alias for this session. end this session
        $kernel->alias_remove($heap->{alias});

        # spawn a NS component
        NS->spawn({
            alias   => $heap->{alias},
        });
    ...
    # if a SB command
    if ($data =~ /Hello SB/) {
        $logger->info("dispatching further requests to SB");

        # unset the alias for this session. end this session
        $kernel->alias_remove($heap->{alias});
        #undef $heap;

        # spawn a SB component
        SB->spawn({
            alias   => $heap->{alias},
        });
